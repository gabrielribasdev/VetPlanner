...
const Historico: React.FC = () => {
  ...
  const totalPaginas = Math.ceil(agendamentosFiltrados.length / ITENS_POR_PAGINA);
  const inicioIndex = (paginaAtual - 1) * ITENS_POR_PAGINA;
  const fimIndex = inicioIndex + ITENS_POR_PAGINA;
  const agendamentosPaginados = agendamentosFiltrados.slice(inicioIndex, fimIndex);
    ...
    <Layout>
      <Container>
        ...
        <Wrapper>
          ...
          <Select value={filtroPet} onChange={(e) => setFiltroPet(e.target.value)}>
          ...
          agendamentosPaginados.map((agendamento, index) => (
            <Wrapper key={index}>
              <Item>
                ...
                <Info>
                  <strong>Preço:</strong> R$ {parseFloat(agendamento.preco).toFixed(2)}
                </Info>
              </Item>
            </Wrapper>
          ))
        ...
        {totalPaginas > 1 && (
          <PaginationWrapper>
            <PaginationButton onClick={() => mudarPagina(paginaAtual - 1)} disabled={paginaAtual === 1}>
              Anterior
            </PaginationButton>
            <PageIndicator>
              Página {paginaAtual} de {totalPaginas}
            </PageIndicator>
            <PaginationButton onClick={() => mudarPagina(paginaAtual + 1)} disabled={paginaAtual === totalPaginas}>
              Próxima
            </PaginationButton>
          </PaginationWrapper>
        )}
